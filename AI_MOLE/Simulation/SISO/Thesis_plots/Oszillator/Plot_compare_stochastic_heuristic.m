...
args.x_cell = {iter_vec, data_stochastic.t_vec};
...
args.x_label_cell = {'Iteration', '$t$ in $\mathrm{s}$'};
args.y_label_cell = {'RMSE in $\mathrm{m}$', '$y_{L,10}$ in $\mathrm{m}$'};
args.title_cell = {'\textbf{(a)}', '\textbf{(b)}'};
args.legend_cell = {{'MOLE', 'MOLEs', 'NOILC'}, {'Soll', 'MOLE', 'MOLEs', 'NOILC'}};

args.filename = fullfile('05_Ergebnisse_Diskussion', 'Ergebnis_Osz_linear_vergleich_stochastic.pdf');
