...
if ~isfile(data_name)
    ...
    % Initialize MOLE-Cell
    SISO_MOLE_Cell = cell(3, 1);
    ...
    for j = 1:3
        % Initialize ILC
        params.beta = beta_vec(j);
        SISO_MOLE_Cell{j} = SISO_MOLE_IO(r_vec, u_init, params);
        ...
        for i = 1:params.N_iter
            % Update input
            u_sim = [SISO_MOLE_Cell{j}.update_input(y_sim); 0];
        
            % Simulate the system
            [t_sim, x_sim] = ode45(@(t,x) dynamic_model(t, x, u_sim, t_vec), t_vec, x0, opts);
            y_sim = x_sim(:, 1);
        end
        SISO_MOLE_Cell{j}.save_final_trajectory(y_sim);
        ...
        % GP
        SISO_MOLE_Cell{j}.GP_SISO.L_chol = [];
        SISO_MOLE_Cell{j}.GP_SISO.V_transp = [];
        SISO_MOLE_Cell{j}.GP_SISO.V = [];
        SISO_MOLE_Cell{j}.GP_SISO.GP = [];
    
        % ILC
        SISO_MOLE_Cell{j}.ILC_SISO.W = [];
        SISO_MOLE_Cell{j}.ILC_SISO.S = [];
        SISO_MOLE_Cell{j}.ILC_SISO.R = [];
        SISO_MOLE_Cell{j}.ILC_SISO.L = [];
        SISO_MOLE_Cell{j}.ILC_SISO.Q = [];
        SISO_MOLE_Cell{j}.ILC_SISO.P = [];
    end

    % Save Results
    save(data_name, 'SISO_MOLE_Cell', 't_vec');
